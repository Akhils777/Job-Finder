...
export const jobFinderGetAllApplication = catchAsyncErrors(
  async (req, res, next) => {
    const { _id } = req.user;
    const applications = await Application.find({
      "jobFinderInfo.id": _id,
      "deletedBy.jobFinder": false,
    });
    res.status(200).json({
      success: true,
      applications,
...
export const deleteApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  switch (role) {
    case "Job Finder":
      application.deletedBy.jobFinder = true;
